...
def main(infile):
    ...
    for T in faces:
        ...
        for vee in T.vertices:
            ...
            if key not in vmap:
                ...
                if WITH_TANGENTS:
                    tmp.append(tangents[vi].x)
                    tmp.append(tangents[vi].y)
                    tmp.append(tangents[vi].z)
                vdata.append(tmp)
    ...
    if "map_Bump" in mdict[maxm]:
        ofp.write( ("bump_map "+mdict[maxm]["map_Bump"]+"\n").encode()  ) 
...
if len(sys.argv) == 1:
    ...
    with_tan_cb = tkinter.Checkbutton(win,text="Include tangents",variable=adj_var)
    with_tan_cb.pack()
